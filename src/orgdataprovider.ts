...
export class OrgDataProvider implements vscode.TreeDataProvider<Org> {
    ...
    populateOrgList(): void {
        ...
        const args = ['org', 'list', ...(showExpired ? ['--all'] : []), '--json'];
        const mediaPath = path.join(this.extensionPath, 'dist');
        utilities.loggingChannel.appendLine(`sf ${args.join(' ')}`);
        const iconBuilder: IconBuilder = new IconBuilder(mediaPath);
        execFile('sf', args, { cwd: this.workspaceRoot }, (err, stdout, stderr) => {
            if (err) {
                try {
                    ...
                    vscode.window.showErrorMessage('Error retrieving org list.');
